...
class EmojiManager:
    ...
    def record_usage(self, emoji_id: str):
        ...
        try:
            self._ensure_db()
            self.db.db.emoji.update_one({"_id": emoji_id}, {"$inc": {"usage_count": 1}})
        ...
        try:
            ...
            try:
                ...
                if selected_emoji and "path" in selected_emoji:
                    # 更新使用次数
                    self.db.db.emoji.update_one({"_id": selected_emoji["_id"]}, {"$inc": {"usage_count": 1}})
                    logger.success(
            ...
            for filename in files_to_process:
                ...
                if existing_emoji:
                    ...
                    if not existing_image:
                        ...
                        image_manager.db.db.images.update_one({"hash": image_hash}, {"$set": image_doc}, upsert=True)
    ...
    async def get_random_emoji(self) -> Optional[Tuple[str, str]]:
        """随机获取一个表情包
        Returns:
        ...
        try:
            ...
            # 获取所有表情包
            all_emojis = list(self.db.db.emoji.find({}, {"_id": 1, "path": 1, "description": 1}))
            ...
            logger.error(f"随机获取表情包失败: {str(e)}")
            return None

    async def get_emoji_by_keyword(self, keyword: str) -> Optional[Tuple[str, str]]:
        """根据关键词搜索表情包
        Args:
            keyword: 搜索关键词
        Returns:
        ...
        try:
            ...
            pattern = f".*{keyword}.*"
            all_emojis = list(
                self.db.db.emoji.find(
                    {"discription": {"$regex": pattern, "$options": "i"}}, {"_id": 1, "path": 1, "description": 1}
                )
            )

            if not all_emojis:
                logger.warning(f"未找到包含关键词 '{keyword}' 的表情包")
                return None
            ...
            logger.error(f"按关键词搜索表情包失败: {str(e)}")
            return None

    async def extract_emoji_request(self, text: str) -> Optional[str]:
        """从用户消息中提取表情包需求
        Args:
            text: 用户消息文本
        Returns:
            Optional[str]: 提取出的关键词，如果不是表情包请求则返回None
        """
        try:
            prompt = (
                f"这是用户发送的消息：\n{text}\n"
                "请判断这条消息是否在请求表情包。如果是，请提取用户想要的表情包的关键描述词（情感/内容等）；"
                "如果不是请求表情包，直接回复'否'。\n"
                "示例：\n"
                "- 输入：'我想要个可爱的表情包' -> 回复：'可爱'\n"
                "- 输入：'发个表情包骂人' -> 回复：'生气 骂人'\n"
            ...
            content, _ = await self.llm_emotion_judge.generate_response_async(prompt, temperature=0.3)
            ...
            logger.error(f"识别表情包需求失败: {str(e)}")
            return None
